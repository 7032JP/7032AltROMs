...
- テープからプログラムや機械語ファイルを読み込むとき（`LOAD` / `LOADM` / `RUN "ファイル名"` / `MERGE` / `CHAIN`）、ヘッダブロックの内容（ファイル名・ファイル形式・属性）を 1 行表示してから読み込むようにしました。表示の形は `FILES "CAS0:"` の一覧と同じです（[docs/BASIC_REFERENCE.md](docs/BASIC_REFERENCE.md) §2）。
- `LOADM ,,R` のロード後の実行開始アドレスを、`EXEC` と同じ決め方に揃えました。実行アドレスが 0 で保存された機械語ファイルは先頭のロード先アドレスから実行し、1 バイトも読み込めなかったときは実行へ移らずエラーになります。
- 漢字フォントのライセンスの文書（[LICENSE-FONT.md](LICENSE-FONT.md)）の言い回しを改めました。フォントの出所と権利区分に変わりはありません。
