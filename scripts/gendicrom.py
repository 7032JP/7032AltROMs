...
拡張世代 (AV40EX / AV40SX 相当) の機種構成は、所定の長さの辞書 ROM ファイルが
置かれていることを前提に初期化を進める。本ツールは、その **所定サイズを供給する枠**
として 262,144 byte (0x40000) の全域 0x00 のイメージを生成する。

位置づけは extsub.rom と同じで、機種構成の初期化を成立させるための枠である。
**かな漢字変換 (日本語入力) の機能は実装していない。** 変換辞書としての内容は
一切持たず、日本語入力は非対応である (docs/COMPATIBILITY.md 参照)。

生成物は本ツールが計算で作る単一値の充填イメージであり、機種に元から載っていた
ROM や第三者のソフトウェアのバイト列は一切含まない。同じ引数で何度実行しても
...
def build() -> bytes:
    ...
    # 検査 1: サイズ固定 (所定の長さでないと機種構成の初期化が成立しない)
