...
| `make version` | 配布物のバージョン（`v1.0.1`）と 7T-BASIC のバージョン（`3.1`）を表示する |
...
| `$FBF4` | `BASVEC_SIG` | `7tbasic3` | `boot_bas` / AV 起動用イメージ | 識別用の 2 バイト `"7T"`。一致したときだけ下の 2 つのベクタを読みます。識別語が無い BASIC ROM のときは、コールドスタート `$848B`（BREAK キー ON のときはホットスタート `$8684`）へ移ります |
...
| `$0C00` | FM 音源の音色表（2618 B。34 B × 77、音色番号 0〜76。1 音色は DT/MUL × 4、TL × 4、KS/AR × 4、AM/DR × 4、SR × 4、SL/RR × 4、FB/ALG の 25 B と、LFO 関係の 9 B（Flags・Key Scale Depth・PMS／AMS・AMD・PMD・LFO frequency・Delay Time・Wave Form・Sync Flag。本プロジェクトの表では 0）） | 4 本すべて |
...
期待値はどれも、先に参考書籍の記述だけを根拠に書き下ろし、実行結果と一致したときだけ確定します。実行結果をそのまま期待値にはしません。
