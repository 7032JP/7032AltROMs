...
; 条件・免責は docs/LEGAL.md)。機種識別域 $6B00-$6B13 の内容と AV ブートイメージの
; 位置 $7C00 は当方の ROM 同士の取り決めである。
;
;------------------------------------------------------------------------------
; 機種別変種 (条件アセンブル。ソースは本ファイル 1 本のみ)
;
;   本 ROM が担う起動処理は世代で異なるので、4 通りを作り分ける。
;
...
;
;   INIT_AV1 は AV ブートイメージ (file offset $1C00) を持たず、起動用 ROM イメージを
;   resident boot 領域へ複写してその場で実行する。トランポリンの引渡し先は $FE00。
;
;   FM 音源初期化とアナログパレット初期化は、いずれの変種でも行う。
;==============================================================================
...
reset_entry:
                ...
                ;
                ;   ★ SEG 0 を初期化するには先に $FD90 (MMR セグメント/バンク選択) を
                ;     0 にする。
...
; ★ プリスケーラの設定は必ず先頭で行うこと。
;   $2D / $2E / $2F にはデータビットが無く、レジスタ番号を指定してアクセスするだけで選ばれる。$2D と $2E をアクセスすると
;   FM 部 1/3・SSG 部 1/2 になり、FM-7 シリーズではこれが標準。
;   音階の分周値は f = f_clock / (16 × D)、f_clock = 1.2288MHz で、本機の
;   デバイス供給クロックでこの f_clock を得るには SSG 部を 1/2 に置く必要がある。既定の 1/4 のままだと分周基準が半分になり、同じ D を
;   書いても全音が **1 オクターブ低く**鳴る。
;   この 2 バイトを消してはならない。
;   $21 は触らない。$24〜$26 (タイマの周期) と $27 (タイマと
...
;
;   ★ $6B0E-$6B13 の 6 byte にはコードやデータを置かない。本 ROM 自身はこの領域を
...
;
;   ★ 拡張世代 (INIT_EXSX) の機種区分は上位世代と同じ "40"。世代の違いは
;     $6B09-$6B0D の世代名 ("AV4EX") で表す。
;------------------------------------------------------------------------------
                ZMB     $6B00-*         ; パディング
...
;   $763A から予備。1 エントリの形式:
;     +0〜+24  音源のレジスタへ送る 25 バイト
;     +25〜+33 9 バイト。当方の表は全て 0
;   +0〜+23 はレジスタごとに 4 個ずつの値で、並びはレジスタのオフセット順
;   (スロット 1 / 3 / 2 / 4 = 0 / 4 / 8 / C)。
;   演奏の前に $FD10 でイニシエート ROM を $6000-$7FFF に重ね直し、$6C00 + 34 × 音色番号
;   から 25 バイトを読んで音源のレジスタ $30〜$8C と $B0 に書く使い方ができる。
;   この位置が空だと音色が全て 0 (AR = 0) になり鳴らない。
;------------------------------------------------------------------------------
...
voice_table:
                ...
                FCB     $35  ; FB/ALG
                ZMB     VOICE_LEN-25   ; +25〜+33 の 9 バイト (値は 0)
...
;   転送はラベル経由 (boot_handoff の LDU #bas_image)
...
;   boot_init.rom ($FE00-$FFFF イメージ) を埋め込む。
...
;   当方の ROM 同士の取り決めの入口 (BOOTSVC_RW) から呼ぶ。引数 X = パラメータブロック
