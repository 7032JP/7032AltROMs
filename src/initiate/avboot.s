...
; ROM 同士の内部の取り決めで当方が選んだ位置 (docs/BUILD.md §5.5)。
;
...
;      順に**起動 (IPL ロード) を試行する。本世代の互換要件として、ドライブ 0
;      以外に入れた起動可能メディアからも起動できることが求められる。
;      各ドライブについて
...
;                            止まる (参考書籍)。
;      ★ IPL の置き場は起動モードに依らず $0100 (動作観察)。IPL は $0100〜$02FF の
...
;
; アドレスのピン留めについて:
;   本イメージは入口のアドレスを固定しない (参考書籍に記載が無いため)。FDC は
;   常駐ブートイメージが供給するサービス入口 (BOOTSVC_RW) を呼ぶ。これは当方の ROM
;   同士の内部の取り決めで、機種付属 ROM が使うアドレスとは別に選んである。
...
FBASIC_COLD     EQU     $848B           ; BASIC コールドスタート (同上、A = 0 で ROM モード)
                ENDC
                IFNDEF  STAGE_ALT_RAM
IPL_LOAD        EQU     $0100           ; IPL ロード先 / 実行先 (起動モードに依らず。動作観察)
...
nb_start:
                ...
                ;   そのドライブから起動するのが本世代の挙動 (動作観察): BASIC
                ;   モードでも起動ディスクが入っていればディスクブートが
                ;   優先される。
                LDA     #$01
...
;   おり、ドライブ 1-3 を起動元に指定する手段を持たない。そこで本世代のブート
;   イメージに求められる「ドライブ 0,1,2,3 の順に IPL ロードを試行する」役割を
;   ここで果たす。
;
;   起動セクタ (S1) は探索段階で既に $0100 へ読込済なので、ここでは S2 以降を
;   トラック 0 の 16 セクタまで先読みする。ディスク IPL には、先頭セクタだけで
;   なく後続セクタが $0200 以降に常駐していることを前提に再配置/分岐するものが
;   あるため (resident boot の自動起動と同じ理由)。読めなくなった時点で打切り、
;   読めた分で起動する。
;
