...
;==============================================================================
; AltROMs subsys_c.rom — FM-7 / FM77AV サブ CPU 互換 ROM (Type-C 相当)
;
; 10240 byte 固定、サブ CPU $D800-$FFFF にマップされる。
; サブ CPU が受け持つ画面表示・文字表示・キー入力・カセット入出力・グラフィック
; 描画・タイマ等のサブシステムコマンド、および F-BASIC の表示系コンソール処理まで
; 対応する。ディスク起動・カセット起動いずれのソフトからも利用される。
;
; レイアウト:
...
;
; 設計方針 (独立実装):
;   - 参考書籍に載るコマンド・オーダ・I/O だけを実装する。載っていない位置・
;     振る舞いは持たない
...
;   「実行環境の仕様」は実行環境の外部から観察できる挙動 (動作観察。同 §6.2) を
;   根拠とする事項を指す。
;==============================================================================
...
;   参考書籍に載る位置だけを持つ (参考書籍の $D400/$D401、参考書籍
;   $D409/$D40A/$D40E/$D40F)。
...
;--- システム変数領域 (サブ側ワーク RAM $D000-$D37F) のうち、位置を参考書籍が定めるもの ---
; 参考書籍 (サブシステム変数一覧) は、システム変数の位置を
; Type-A / Type-B と Type-C の 2 通りで定める。同じ働きの変数は同じ名前で持ち、位置だけを
; 組み立ての条件 (SUBSYS_AV = Type-A / Type-B の位置の ROM) で分ける。
;   ・取り消し (CANCEL) の割込みの印 (参考書籍の「アボートフラグ」$FF = IRQ 発生)
;   ・表示開始位置 ($D40E/$D40F) の控え (参考書籍の「VRAM オフセット値」)
;   ・コンソール制御 ($0C) の制御フラグ (参考書籍の「コンソール制御フラグ」)
;   ・タブストップ表 ($0B TABSET の格納先。参考書籍の「TAB 位置設定テーブル」10 byte)
;   ・$04 GET の挿入モード (参考書籍の「インサートモードフラグ」$FF = ON)
;   ・$04 GET の終了キーコード (参考書籍の「GET コマンドの終了キーコード」)
;   ・機能キー行の表示指定 ($01 の相対 8。参考書籍の「PF キー表示フラグ」)
;   ・1 行のバイト数 (参考書籍の「1 行の VRAM 上のアドレス差」)。参考書籍の
;     TEST のサンプルは `LDD $D01F` / `SUBD $D03B` / `STD $D01F` (と $D40E への書込み) で
;     画面を 1 行ぶんずらすので、Type-C の位置はその手順がそのまま動く
;   ・キー入力バッファのワーク (参考書籍: ヘッド = 読出ポインタ 2 byte、テイル = 書込ポインタ
;     2 byte、バッファカウンタ = キーの数 1 byte。KCOUNT / KHEAD / KTAIL)
;   ・割込みを発生する PF キーの番号 (参考書籍の「PF キー割り込み番号」。押されたときに置く)
;   ・タイマ ($3D / $3E) のワーク 18 byte (参考書籍の TMAC / $TC / $T1I / $T2 / $T2D。
;     $TC の次の 4 byte が時刻カウンタ)
; キー押下で入る FIRQ (参考書籍) のハンドラが $D401 のキーコードを読み、32 文字ぶんの
...
; リングの先頭に入る (参考書籍の「先行入力の禁止中はバッファが 1 バイトだけ使われる」と
; 符合する)。
        IFDEF SUBSYS_AV
...
;   F-BASIC (ディスク経路含む) はモニタ選択に依らず同一の共有 RAM コマンド
;   手順 (HALT + $D382=コマンド/$D383-=パラメータ) で受け渡しを行う (実測) ため、
...
;   $39 を ALU とラインドロワで描く。並びと画面は型で分ける (動作観察):
...
;   subsys_c ROM 側のこの範囲は不可視になる (実測)。ここへ実行コードを
...
font_base:
                ...
                ; font.bin は tools/genfont.py で生成 (256 文字 × 8 byte = 2048 byte)。
...
hook_init:
        ...
        ENDC
                LDS     #$D000          ; SP = $D000 (push 時 $CFFF, $CFFE... と下方に伸びる。
                                                ;   共有 RAM ($D380+) と衝突しない)
                                                ;   S の最初の設定は LDS の即値で行う。実行環境はこの
                                                ;   「設定」を LDS の即値でだけ認めるものがあり、TFR で
                                                ;   S を作ると周期 NMI (タイマの計時) が一度も入らず、
                                                ;   READ TIMER ($3E) の待ちが終わらなくなる (動作観察)
        ...
        ENDC
                STX     <WK_LINE_BYTES
        IFDEF SUBSYS_AV
                JSR     av_view_full            ; 描画範囲 = 画面全体 ($39 が変える。動作観察)
        ELSE
                LDD     #639
                STD     <WK_VIEW_X1
                LDD     #199
...
main_loop:
                ...
                ; コマンド待ち — 参考書籍の手順:
                ;   ① $D380 ATN ビットをクリア (応答受領)
                ;   ② $D382 (SH_CMD) != 0 なら HALT の間に置かれたコマンド → BUSY のまま dispatch
                ;   ③ $D40A を読んで BUSY クリア (= サブ レディ) し、$D382 を読む
                ;   ④ $D382 != 0 ならコマンドあり → dispatch
                ;   ⑤ $D380 bit7 (ATN) 立ったらループ先頭へ
                ;   ⑥ それ以外は $D382 を再読み
wait_cmd:
                ; ①〜⑥ の手順は分割転送の待ち (sh_rereq) と同一なので 1 本に
                ;   まとめてある。復帰時 A = コマンドコード、BUSY はセット済。
                JSR     sh_rereq
dispatch_cmd2:                                  ; BUSY セット済の再ディスパッチ入口
                CLR     -1,U                    ; 受理した時点で消費済にする
                ; 共有 RAM 相対 0 は「READY REQ (MSB) / ERROR CODE」(参考書籍)。
...
;   一覧に載る区間だけを並べ、どの区間にも入らないコードは参考書籍の
;   「コマンドコードに誤りがあります」($46) として返す。
...
;------------------------------------------------------------------------------
;--- 時刻カウンタの桁上限表 (0 詰め跡の空き領域へ配置) ------------------------
;   周期処理の桁送り経路だけが参照する 4 byte。参照は即値アドレスなので置き場所は
;   自由で、周期処理の区画を空けるためここへ移してある。
nmi_climits:
...
cmd_dispatch_table:
        ...
        IFDEF SUBSYS_TB
                FDB     c_nocode                ; $1B Type-B の位置では受け付けない ($46。動作観察)
                FDB     pb1_fast        ; $1C PUT BLOCK 1
                FDB     gb2b_entry              ; $1D Type-B の位置の矩形読出し 2 (動作観察)
        ELSE
        ...
        ENDC
        IFDEF SUBSYS_TB
                FDB     tb1e_entry              ; $1E Type-B の位置の矩形書込み 2 (動作観察)
        ELSE
        ...
        IFDEF SUBSYS_AV
                ...
                FDB     line15_entry    ; $36 LINE 2 (参考書籍: 従来のソフトウェアに
                                                ;   よる直線。本 ROM は $15 と同じ経路 (ラインドロワ) で引く。
                                                ;   補間の違いは持たない)
...
;--- c7f_entry — $7F 共有 RAM 相対 3 からの処理の実行 (動作観察) -----------------------
;   転送された処理の中では取り消しの IRQ を受けない (受けるかどうかは転送された処理が
;   決める。受けたときは $D402 を読み印を置いて戻るだけ = 動作観察のまま)。
...
;   pt_fill と pt_lov が同じ 4 命令を持っていたので 1 本にまとめてある。
pt_hln:
                STD     <WK_LN_X1
                LDD     <PT_CY
                STD     <WK_LN_Y0
...
;------------------------------------------------------------------------------
; getpf_entry — Get String of PF ($2B, GSTRPF)。入口の受け渡しは動作観察による。
;   $D383=NO。PFキー定義テーブル (KB_PF_BASE+NO*16) から 16byte を $D384〜$D393 へ転送。
;   先頭バイトは参考書籍 応答データ 相対 4 の N (文字数) そのもの。
;------------------------------------------------------------------------------
getpf_entry:
...
;   参考書籍の (2) Define String of PF / (3) GET String of PF は
;   PF キー番号 NO = 1〜10、定義文字数 N = 0〜15 と定める。範囲を外れた時は
...
;------------------------------------------------------------------------------
; funkey_entry — Define String of PF ($2A, FUNKEY)。入口の受け渡しは動作観察による。
...
;==============================================================================
; CONSOLE の機能キー行表示 (参考書籍「CONSOLE 文」の第 3 引数。
; 参考書籍の相対 8 (FD))
;==============================================================================

;--- pf_show — 機能キー行 (画面の下から 2 行) を描く --------------------------------
;   $01 の相対 8 (FD) が非 0 のときだけ描き、0 なら何もしない (参考書籍のシステム
;   参考書籍「CONSOLE 文」)。参考書籍が定めない
;   細部は次のとおり定める:
...
;   参考書籍 ページリミット: 「ページモード画面に表示中に、画面の最終文字
;     位置に文字を表示した時、あるいは、最終行表示中に LF オーダーを受けつけた
;     時に、ベルを鳴らして表示動作を停止します。停止状態を解除するには [BREAK]
;     および [PF] を除く任意のキーを押します。ただし、コンソール制御でページ
;     ウェイトが選択されていない場合は、上記動作は行なわれません」。ページ
;     ウェイトはコンソール制御 (参考書籍) の CF bit3。
;   参考書籍 表示停止 ([ESC] によるプットウェイト) も解除条件は同じなので、
;     解除待ちだけを con_wait_key として共用する。
;   [BREAK] はキーバッファへは入らず、[PF] キーも参考書籍の
;     とおり「文字列の生成」か「割込みの発生」のどちらかになり、PF のキーコード
;     そのものはバッファへ入らない。よってバッファから取り出せたキーはいずれも
;     解除キーである。停止するときはベルを鳴らす ($D403 の読出し。参考書籍
;     )。
...
;   F-BASIC サブ参考書籍 (参考書籍): コンソール入力位置 (カーソル座標) を返す。
;   $D383 = カーソル列 (X), $D384 = カーソル行 (Y)。メイン側はこの戻り値で
;   コンソールカーソルを追跡し、後続のカーソル制御に用いる。
;------------------------------------------------------------------------------
                ; 本体は c0a_ext (コード域)。行はスクロール範囲相対で返す。
                ; ※ かつてここに置いていた 4 byte の 0 詰めは、$03 出力中の
                ;   カーソル退避のための区画の予算に充てて撤去した。この区画は
                ;   $E1FF ピンまでで独立しており、他の区画へは影響しない。

;------------------------------------------------------------------------------
; cmd_0b_tabset — TABSET ($0B、実行環境の仕様)
;   $D383〜 10 byte = 80 桁ぶんのタブストップビットマップを、TAB_TABLE
;   (サブ側ワーク RAM の内部配置) へ複写する (F-BASIC がブート時に毎回発行)。
;------------------------------------------------------------------------------
cmd_0b_tabset:
...
;==============================================================================
; line15_entry — cmd $15 LINE 本体 (実行環境の仕様のレイアウトに準拠、ROM 末尾空き領域)
;   入口のパラメータ配置:
...
;   範囲外座標の扱い (是正): 端点座標が 1 語でも画面外なら、この命令は
...
;
;   オペレータ制御は参考書籍に従う。
;     (1) 移動キー … カーソルを 1 ステップ (1〜10 ドット) 指定方向へ動かす
;     (3) 数字キー … 1 ステップの移動ドット数を指定する。0 は 10 ドット、
;                    1〜9 はその数字のドット数。初期値は 1
;     (4) 改行キー … 座標の読取りを行なう。指定された数の座標を読取った時に
;                    本コマンドを終了する
;   参考書籍の [SHIFT] 併用 (20 ドット単位) は、参考書籍のキャラクタ
;   コード表が移動キーに 1 つのコードしか割当てておらず、参考書籍が示す
;   キーコードの読出し口にも SHIFT の状態が現れないため、参考書籍の記述だけでは
;   通常の移動キーと区別できない。区別が付かない以上、憶測で別の動きを与えず
;   1 ステップの移動として扱う。
;
...
;   同じ 4 組を同じ順で受けるので、同一の 20 byte を各所に並べる代わりに
;   1 本にまとめてある (区画の予算をここで捻出する)。
...
rect_from_param:
                ...
                STD     <WK_RECT_Y1
                RTS
;--- (cursor_show / cursor_hide は ROM 末尾のコード域へ移設 —
;     scroll_up の高速化 (面別 4 倍展開) の場所を空けるため) ---
...
;   相対 3,4 = IC (16 ビットの割込制御フラグ)。「16 ビットのうち、下位 10 ビットを、
;   それぞれ PF キーに対応させてあり、1 になっているビットの PF キーからの割込みを
;   受けつけます」。ビット割付は bit0=PF1 … bit9=PF10。
...
;==============================================================================
; コンソール制御コード拡張 (ROM 末尾の空き領域に配置、直後の ZMB が吸収)
;   既存末尾ルーチン群の後ろ (= 最後尾) に置き、既存コードのシフトを避ける。
;==============================================================================
...
;                      / $1B=Lock Keyboard 等の 2 バイト目 (読み捨てる。#586)。
...
;   0 のまま (動作観察。STAGE_ALT_RAM はオーダシーケンスの誤り $3E (参考書籍) を置く)。
;   先行入力の許可 / 停止 (同 (27)(28)) は状態を保つ。参考書籍には、停止したときに
;   キーバッファのキーコードを捨てるとは書かれていないので捨てない (捨て去るのは
;   Erase Key Buffer (同 (26)) と $29 INKEY の RESET フラグ (参考書籍) である)。
pe_esc2:
                CMPA    #$39                    ; Erase Key Buffer (同 (26))
                BEQ     pe_es_erase
                LDB     #$80                    ; Lock / Unlock Keyboard のビット
                CMPA    #$23                    ; Lock Keyboard (同 (24))
                BEQ     pe_es_set
                CMPA    #$22                    ; Unlock Keyboard (同 (25))
                BEQ     pe_es_clr
                LDB     #$01                    ; 先行入力のビット
                CMPA    #$67                    ; Set Buffer Mode (同 (27))
                BEQ     pe_es_set
                CMPA    #$68                    ; Set Unbuffer Mode (同 (28))
                BEQ     pe_es_clr
        IFDEF STAGE_ALT_RAM
                LDA     #$3E                    ; オーダシーケンスの誤り
                STA     >SH_ERR
...
pe_rcx:
                ...
                ; $16 CHAIN — N 点ポリライン。本体は ROM 末尾空き領域 (chain16_entry)。
                ; 「前回終点連結 + 範囲検査なし」の形は実行環境の仕様と別物であり、
                ; 範囲外座標の打点が VRAM 外 (共有 RAM) へ流出し得るため採らない。
...
putchar:
                ...
                ;   フォントの字形として 1 桁ぶん描く (公開仕様の表示状態切替)。
...
;   「バッファアドレスを現在の行の先頭へ移動します。この時に、コンソール制御で
;   オート LF が選択されている場合には、LF オーダの動作も同時に行ないます」。
;   オート LF はコンソール制御 (参考書籍) の CF bit5。
putchar_cr:
                CLR     <WK_CURSOR_X
                LDA     <WK_CSCTL       ; 表示制御ワーク (CF の現在値)
                BITA    #$20                    ; bit5 = オート LF
...
pc_bs:
                ; BS オーダ (参考書籍): 「現在のバッファアドレスが、フィールドの
                ;   先頭でないならば、バッファアドレスを 1 つ前に戻します。フィールドの
                ;   先頭の場合は、何も動作しません」。桁の先頭ではなくフィールドの
                ;   先頭が境界なので、前の行の末尾へも戻る。
...
;   「現在のモード画面を全て Null コード ($00) にして、バッファアドレスを
;   その先頭へ移動します」。B (呼出元 cmd_03_console の文字数カウンタ) と
...
;------------------------------------------------------------------------------
; cmd_20_chline — Character Line (LINE@ 文字形式、実行環境の仕様)
...
;   ビット FIFO の詰め込みと行頭位相合せが同じ展開列を各々持っていたので
;   1 本にまとめてある。
d_rsh8:
                LSRA
                RORB
...
;   (get_pixel_color) が同じ計算を持っていたので 1 本にまとめてある。
;------------------------------------------------------------------------------
;--- d_lsr3 — D を 3 ビット右へずらす (座標 → バイト単位) ----------------------
;   同じ 6 命令の並びが図形・矩形書込み・塗り潰しの各所に散っていたので
;   1 本にまとめてある。D 以外のレジスタは触らない。
...
;   水平線を 1 ドットずつ pset_internal (約 78 命令/ドット) で描くと $15 形状 2
;   (塗潰し矩形) の全画面塗りが 33 秒超かかるため、専用経路を設けてある。
...
;   BSR/PSHS を一切使わない (スタック追加消費 0)。描画最深部に NMI が
;     重なってもサブ CPU スタックが浅いまま保たれ、他のプログラムが $3F で常駐させた
;     データ域 ($CFC7-) を踏まない。
;   ワークは直線 DDA 用スクラッチ ($D064-) を流用 (水平経路では未使用)。入口の
;   draw_line が直接ページ $D0 を確立済みなので、そのまま直接アドレッシングで届く。
;   左端 X の 1 語だけは PSET 用 X 座標 ($D070-$D071) を借りる。本経路は 1 dot
...
;   左端と右端で同じ式 (new = (old & ~m) | (充填値≠0 ? m : 0)) を使うので
;   1 本にまとめてある。入力: U = B プレーン内アドレス / HL_T = 立てるビット /
...
;------------------------------------------------------------------------------
                ...
                ; 検証に合格するまでワークへ一切書かない方式をとる。独自プロトコルの
                ;   値 ($D384=62 等) を載せた呼出でモード変数が壊れないようにするため
                ;   (本体コメント参照)。
...
;==============================================================================
; chl_disp — $20 Character Line ボックスフラグ ($D38D) 分岐 (実行環境の仕様)
...
;   文字の記録 (draw_char_at_cursor) と行消去 (EL オーダ) が同じ計算を持って
;   いたので 1 本にまとめてある。A,B 破壊。
con_cell:
                LDA     <WK_CURSOR_Y
                LDB     <WK_SCREEN_W
                MUL                             ; D = 行 × 桁数
...
;   同じ 3 命令が cl_plot / cb_next_cell にもあったので 1 本にまとめてある。
cb_mkx:
                ADCA    #0
...
;   BS (オーダ・キーの双方) が「フィールドの先頭では何もしない」を判定する。
...
;       その F を消し、そのフィールドの残りにも同じ属性を及ぼす (仕様の例 ③)。
;     ・一連の外 … セルのアトリビュートのまま。
...
;   アトリビュートの bit3 = R (参考書籍) が 1 のセルは「文字の色を反転
;   表示」する。字形のビットを反転させてから同じ経路へ流すので、字形の部分が
...
;   字形描画 (draw_char_at_cursor) と反転 (cursor_toggle) が同じ計算を持って
;   いたので 1 本にまとめてある。途中結果はスタックに置く。A,B 破壊。
cell_vaddr:
                LDA     <WK_CURSOR_Y
...
;   公開仕様どおりの見え方である。字形は 8 ライン ぶんしかないので、行ピッチが
...
cursor_toggle:
                ...
                ; 反転する高さは行ピッチに合わせる。文字セルは 25 行で 8 ライン、
                ; 20 行で 10 ライン取るので、常に 8 ラインで止めると 20 行のとき
                ; だけセル下端 2 ラインが反転から漏れる (参考書籍のカーソルは
                ; セル全体を反転させたもの。動作観察でも 20 行では 10 ライン)。
...
;   参考書籍の EDIT 文は「カーソルが行番号の直後で
;   点滅します」と定める。周期の値は書籍に定めが無いので、目で追える
;   速さとして 0.4 秒ごとに反転を入れ替える (= 0.8 秒の周期) を採った (動作観察)。
...
;--- パターン OR/AND/EOR 合成の 3 入口は撤去した ---
;    ソース中に呼出しが 1 件も無く (定義行以外の参照 0、直前は RTS なので
;    落込みも無い)、FM-7 構成でしか組まれていなかった区画である。空いた区画は
;    コンソール制御 (参考書籍) と出力の継続 (参考書籍) の実装に充てる。
...
;   ストローブが消える)。通常のキーはリングへ。PF キーは参考書籍
;   とおり「文字列の生成」か「割込みの発生」のどちらか (2.2.13(4) の
;   割込制御フラグで選ぶ)。割込みの発生は、キーが届いたその場で STATUS (共有 RAM
;   相対 1) の bit0〜3 に PF 番号を置き (参考書籍)、ATTENTION (参考書籍。$D404 の
;   読出し。参考書籍) を出す。割り込まれた場所 (スタックの PC) が
...
hdlr_FIRQ:
                ...
                IFNDEF  STAGE_ALT_RAM
                BNE     fr_key                  ; bit0 = 先行入力 (同 (27)) の間はリングに溜める
                LDX     #KB_BUF_LO              ; 停止中 (同 (28)) はリングを空にしてから格納する
                CLR     <KB_COUNT
                STX     <KB_RDPTR
                STX     <KB_WRPTR
fr_key:
...
pb_passinit:
        ...
        ENDC
;==============================================================================
; N6: $1C/$1E 矩形書込み 高速化 — バイト単位ブリット (ROM 末尾空き領域)
;
...
;   作業域はスタックフレーム (LEAS -28,S) ではなく固定 RAM に置く。描画最深部に
;   NMI が重なるとスタックが $CFC7 まで伸び、他のプログラムが $3F で常駐
;   させたデータ域を踏み潰すためである (コマンドは直列実行で再入なし)。
...
pbf_guard:
                ...
                BCS     pbg_bad                 ;   (同じ 4 段の検査を 1 本にまとめた)
...
;   ビットを詰めるので進めない。$1C と $1E で同じ並びだったので 1 本にまとめた。
pbb_rowloop:
                JSR     pbb_row
...
;   各プレーンはバイト境界から開始 (実行環境の仕様) — パス毎に FIFO を空にして
...
;   (C byte) 進める。$1C と $1E で同じ並びだったので 1 本にまとめてある。
pbf_rowloop:
                JSR     pbf_row
                LDD     <PBF_ROWVR              ; 次行へ (+80 byte)
                ADDD    #80
...
pt_scan_rf:
        ...
        ELSE
                ...
                STX     <PT_DIR                 ;   (実行環境はリミット列で打ち切るが、
                LDX     #PT_COLS*256+$FF        ;   ヒット有無の判定は呼出側比較で等価)
...
;        凹形状・渦巻も完全充填する
;     6. シードスタック満杯時は黙殺 (取り残しが出得るのは実行環境の仕様でも同じ)
;   実装方式:
...
PT_CY           EQU     PT_B+10 ; 2: 現処理行 Y (PT_TX と 2 語連続 = 種点の転記先)
...
pta_new:
                ...
                STD     <PT_TX
        ENDC
;   種点が描画範囲 (WK_VIEW_*) の外なら $3F (動作観察)。範囲の左右の縁のバイト列と、
...
ptw_1:
        ...
        ELSE
                ...
                ;   (動作観察。STAGE_ALT_RAM は参考書籍の 1〜8 の外を $43)。
                LDB     5,U                     ; N = 境界色の数
        IFDEF STAGE_ALT_RAM
                DECB                            ; 参考書籍の Nc は 1〜8
                CMPB    #7
...
pt_exc2:
                ...
                ;   PT_TX (シード X) と PT_CY (処理行 Y) は 2 語連続に配置して
...
;   入力 A = 再開タイプ (0/1/2)。満杯時は黙殺 (実行環境の仕様と同じ)。
pt_push:
                LDX     <PT_SP
                CMPX    #PT_STK_BOT
                BEQ     pt_puf
...
;   参考書籍の相対 1 は継続フラグビット S′ (MSB) である。「出力する
;     文字列が 124 バイトを越える場合や、処理の都合上分割したい場合には、
;     S′(継続フラグビット) を 1 にして、コマンドを送り、以後、CONTINUE (継続)
;     コマンドを用いて、残りのデータを転送します」。継続コマンド (参考書籍)
...
cmd_03_console:
                ...
                ; --- 表示停止 (参考書籍) ---
                ;   「[ESC] キーが入力されると、現在依頼されている文字列を表示した
                ;   後、新たにキー入力がされるまで PUT コマンドの状態で待機します」。
                ;   「ただし、コンソール制御でプットウェイトが選択されていない場合は、
                ;   上記動作は行なわれません」= CF bit4 (参考書籍)。
                LDA     <WK_CSCTL       ; 表示制御ワーク (CF の現在値)
...
;   カーソル指定 $00/$FF) で共用する。どちらも「有効なら出す、無効なら消す」で
;   同じ処理なので一本化した (区画の予算上も、共用にして初めて $03 側が収まる)。
...
;
;   F-BASIC の SYMBOL 文 (公開仕様: 参考書籍) の
...
;   0〜199 を外れると $3F。点はラインドロワで 12 の面へ打つ (動作観察)。
        ...
        ENDC
                ; --- パラメータ取り込み ---
                LDA     9,U                     ; $D38C = 文字数 (動作観察: 0 は何も描かず、81 以上も誤りにしない)
...
;------------------------------------------------------------------------------
                ; 本体は ROM 末尾空き領域 (line15_entry)。実行環境の仕様のパラメータ配置
                ; ($D383=色 / $D385-=X0,Y0,X1,Y1 / $D38D=形状) に準拠した実装へ刷新。
                ; 座標の読出位置を 2 byte でも取り違えると、誤った Y の打点が VRAM 外
                ; ($C0xx 共有 RAM) へ流出して常駐コードを壊すため、配置に厳密に従う。
...
; VRAM 1 byte 論理合成の 4 入口 (OR / なし / AND / EOR) は撤去した。
;   ソース中に呼出しが 1 件も無く (定義行以外の参照 0、直前は無条件分岐なので
;   落込みも無い)、FM-7 構成でしか組まれていなかった区画である。空いた区画は
;   コンソール制御 (参考書籍) と出力の継続 (参考書籍) の実装に充てる。
;   AV Type-A 構成にはもともとこの区画が無く、同じ跡地を $22 が使っているため
;   AV 側の配置は本撤去で 1 byte も動かない。
;------------------------------------------------------------------------------

;--- fig_color / fn_color — 色 + ファンクションコードの確定 (共通処理) ---------
;   fig_color は相対 3 = CL / 相対 4 = F の配置 (参考書籍の (1) LINE /
;   (2) CHAIN / (5) SYMBOL) をそのまま読む入口。fn_color は A = F / B = CL を
;   直接受ける入口で、点ごとに CL と F を持つ参考書籍 POINT が使う。
...
;         Y = 書き先 (同じ並びの 2 語)
;   出力: C=1 … 画面内。2 語とも転記済で X/Y は 4 byte 進む
;         C=0 … 画面外。違反語は転記せず打切る (呼出側は描画を取止める)
...
;   受け皿。実行環境の仕様のビット連続詰め (行末でバイト境界へ切上げない) を
...
line15_entry:
                ...
                ; 座標 4 語 ($D385=X0, $D387=Y0, $D389=X1, $D38B=Y1) を範囲検査
                ; しながら WK_LN_X0/Y0/X1/Y1 (連続 4 語) へ取り込む。1 語でも
                ; 画面外なら何も描かずに正常終了する。
                LEAX    2,U
                LDY     #WK_LN_X0
l15_chk:
...
l15_copy:
                ...
                CMPA    #1
                BEQ     l15_box
                ; D38D>=2 = 塗潰し矩形 (BOXFILL)。実行環境の仕様の形状値であり、
                ; 楕円ではない。
...
;------------------------------------------------------------------------------
; point_entry — POINT ($17)。入口の受け渡しは動作観察による。
...
tpt_fn:
        ...
        ELSE
                ...
                LDY     #WK_PX_X                ; WK_PX_X/WK_PX_Y は 2 語連続
                JSR     gr_xy                   ; 範囲検査つき転記
...
pt_done:
bc_stop:
                RTS                 ; 直前の JMP cmd_stub_ok を共用                             ; (元: BRA tc_done)
...
;   メイン側のプログラムがサブ CPU 側に独自ルーチンを転送して
;   実行するために使う。アップロードは事前にメイン CPU がサブ CPU を
;   HALT させて共有 RAM ($D000-$D37F 等) 経由でコードを書き込む流儀。
;
;   注: 飛び先は本 ROM 外 (典型的には $D000 系の RAM 領域)。RTS で戻る
;       前提の場合と、JMP で帰ってこない場合がある。本実装は呼び側で
;       JSR ではなく JMP で渡すことを前提とする (= 戻ってこなくても OK)。
;------------------------------------------------------------------------------
;------------------------------------------------------------------------------
; cmd_3f_bytecode — 拡張コマンド $3F のバイトコードインタプリタ
;
; メイン CPU が $D38B-$D3FF にバイトコード列を配置し、本ハンドラが解釈実行する。
; これにより他のプログラムは「memcpy で自前 6809 ルーチンをサブ CPU ワーク RAM に
; 展開、その後 CALL で実行」のような複合操作を 1 つの $3F コマンドで実現できる。
;
; オペコード (参考書籍と動作観察に基づく):
;   $90        : STOP (RTS)
;   $91 src dst len : MEMCPY (各 16-bit BE)
;                     例: $91 D3 94 C8 00 00 56 → src=$D394, dst=$C800, len=$0056
...
;------------------------------------------------------------------------------
; hdlr_NMI — 周期 NMI (参考書籍): タイマ機能 (参考書籍) の計時
...
;==============================================================================
; chain16_entry — cmd $16 CHAIN 本体 (実行環境の仕様のレイアウトに準拠)
...
;==============================================================================
; キャラクタブロック GET@/PUT@ ($06-$09) — 実行環境の仕様の正規実装
...
;   - 矩形書込み 1: 単色ビットイメージを矩形へ書込 (本実装は SET function のみ)
;   - 矩形読出し 2: 矩形の 3 原色プレーン (B/R/G) をそのまま読出
;   - 矩形書込み 2: 3 原色プレーンを矩形へ書込 (本実装は SET function のみ)
;
;   制限事項:
;   - 矩形書込み 1 の function code は SET (0) のみ対応。
;   - 矩形書込み 2 は SET (0) に加え OR (2) に対応 (演算指定の値。
;     bit=1 のドットのみ点灯し bit=0 は画面を保持する)。PRESET/AND/XOR は
;     SET と同扱い (将来拡張)。
;
;   応答プロトコル (矩形読出し 1・2 共通):
;   - $D383 = 当該チャンクのデータ計数、データ本体は $D384 から (実行環境の仕様)
...
;   両端点を検証後、桁/行とも min/max へ正規化する (実行環境の仕様として逆順指定も許容)。
cb_parse:
                LDD     0,U                     ; A=X1, B=Y1
                BSR     cb_valid
                BCC     cbp_rts
...
;   が受け持つ。座標範囲外は実行環境の仕様のエラー条件であり「描かず正常応答」
;   ($15/$16 と同方針)。
;------------------------------------------------------------------------------
...
gb_flush_done:
                RTS
gb_endrow:
                ; 行末はバイト境界へ切上げず次行へ連続 (実行環境の仕様のビット連続詰め)
                LDX     <WK_PX_Y
                LEAX    1,X
                STX     <WK_PX_Y
                BRA     gb_yloop
...
;==============================================================================
; $20 Character Line — 文字でラインを引く (LINE@ 文字形式、実行環境の仕様)
;   $D383=属性, $D384=文字コード,
...
;   属性は既存セルの bit7 (論理行頭マーカ) を保存して合成する (実行環境の仕様)。
;==============================================================================
chline_entry:
                LDA     3,U                     ; $D386 = X0
                LDB     5,U                     ; $D388 = Y0
...
;   $06/$08/$1B/$1D の入口が同じ 3 命令を持っていたので 1 本にまとめてある。
cb_resp0:
                CLR     0,U
                LEAX    1,U
                STX     <CB_RPTR
...
;   いずれも受理の後に相対 0 を 0 へ戻し、$D381 は bit7 だけを落とす
;   (ステータスの低位は保持)。A 破壊。
sh_cont_set:
                LDA     >SH_CNT
                ORA     #$80
...
sh_cont_ack:
                BSR     sh_rereq_c              ; 受理済コマンドを清算して (BUSY 中で
                                                ;   安全) BUSY を落とし次を待つ
                CLR     >SH_ERR
                LDA     >SH_CNT                 ; bit7 のみ落とす (ステータスは保持)
                ANDA    #$7F
                STA     >SH_CNT
                RTS
...
;   実行環境の仕様: $D38D = 本パケットで供給されたパターン byte 数。ブロック全体に
;   満たない場合、サブは BUSY を落として次チャンクを要求し、メインは継続
;   コマンド $64 ($D383=計数, $D384〜=データ) を書いて分割転送を継続する。
...
pbs_refill:
                ...
                LDA     >SH_PARAM               ; 新チャンク: $D383 = 計数
                STA     <PBS_CNT
                LEAX    1,U             ;             $D384〜 = データ
                STX     <PBS_PTR
                BRA     pbs_loop
pbs_next:                                       ; A = 次パターンバイト
                ; 分割受取の入口 (pbb_fetch/pbb_take 系) からは、アップロードされた
                ; 機械語が DP=$D0 のまま JSR してくることがある。
                ; 本ルーチンは作業 RAM を DP=$D0 で直接参照するため、呼出元の DP を
                ; 問わずここで確立し、復帰時に呼出元の DP を戻す。
                PSHS    DP,X
                LDA     #$D0
                TFR     A,DP
pbs_loop:
                LDA     <PBS_CNT
                BEQ     pbs_refill              ; チャンク枯渇 → 継続要求
                DEC     <PBS_CNT
                LDX     <PBS_PTR
                CMPX    #$D400
                BCC     pbs_guard               ; 終端越え = 枯渇扱い
                LDA     ,X+
                STX     <PBS_PTR
...
;   済み。送信側 (パターン分割受取) と受信側 (応答分割提示) で同じ手順なので
;   1 本にまとめてある。
...
;   (動作観察: 20 行モードでの範囲 16,4 は y=160-199、25 行モードでの範囲 16,4 は
;   y=128-159、範囲 20,4 は y=160-191。いずれも 上端 × 行ピッチ が区画の上端)。
...
c0d_ext:
        ...
        IFNDEF SUBSYS_TB
        IFNDEF STAGE_ALT_RAM
c02_ext:                                ; $02 も W = 0 のときだけ BC を背景色にする (動作観察)
        ENDC
        ENDC
                LDA     0,U                     ; W
        IFDEF SUBSYS_TB
                CMPA    #2
                LBCC    c_e3c                   ; Type-B の位置: W = 2 以上は $3C (動作観察)
                TSTA
        ENDC
...
;   画面の終り (8000) なら面の終り ($2000) まで埋める (動作観察)。
                CMPD    #8000
                BNE     tvf_e
                LDD     #$2000
tvf_e:
...
;     上へメモリ移動し、範囲最下行をクリアする (動作観察: 範囲外の図形は動かない)。
...
;   F-BASIC の CLS はこのコマンド ($D383=0 で全画面消去) を発行する。
...
;   挿入し、実行環境のビット連続詰めをバイト境界出力へ位相合せする (ダミーは端マスク
;   で必ず落ちる)。
...
;   公開 参考書籍の PUT@ 形式2 は動作型 PSET / OR / AND / XOR を採り、
...
;   いる場合のみ適用し、それ以外は無修正の汎用ループ (prw_mid) へ落ちる。
...
;       ($05 は上位・下位のどちらかが $00 のとき、キーエンコーダ側が FM-7 と同じ
;        既定の時間 (開始 0.7 秒 / 動作 0.07 秒) を採る。当 ROM は値をそのまま渡す)
;   キーエンコーダ自体への命令の並びは公開資料に無いので、副命令とパラメータを
;   データレジスタ $D431 へ 1 バイトずつ書き、書くたびにステータスレジスタ $D432 の
;   bit0 (ACK。0 = 応答中) が 1 に戻るまで待つ (動作観察)。
...
;   相対 0 に $45 (参考書籍「コマンド、パラメータの誤り」) を置く。
...
AV_PG0          EQU     $D0E8   ; 1: 描画中に $D430 へ書く値 (bit1 を立てる。Type-B の位置は控えの bit5 も落とし、640×200 は $22 の処理ページのまま描く。動作観察)
...
;   av_fill / av_seg / $39 が同じ並びを持っていたので 1 本にまとめてある。
av_xy2:
                TFR     X,Y
av_xy2y:
                BSR     av_xy
...
amd_bad:
                ORCC    #$01                    ; C=1
                BRA     amd_x
;--- amd_abs — U の指す語を絶対値にする。D = 絶対値 (Z = 0 のとき) ---------------------
amd_abs:
                LDD     ,U
                BPL     ama_r
                NEGA                            ; D = -D
...
;   相対 0 に $46 (参考書籍「コマンドコードに誤りがあります」) を置く
;   (動作観察)。
c_e3c:
                LDA     #$3C                    ; $3C (動作観察)
...
;   $0000・$2000 の面にある (320×200 の画面。動作観察)。応答は相対 3 = 計数、相対 4 から
