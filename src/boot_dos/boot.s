...
; の存在を定め、参考書籍がその動作を記す。モードは本体背面のモード選択
; ディップスイッチで選ぶ。このスイッチはブート ROM の A9・A10 を与えて $FE00 に現れる
; 区画を選び、同時に ROM/RAM 切換回路へも入る (参考書籍)。
; したがって、
;   ・FM-7 では、どのモードで起動したかを読み取れる I/O が参考書籍に無い (I/O
;     アドレスマップに該当のビットが無い)。モードごとに別のブート ROM が用意される
;     (FM77AV 系の $FD0B は本 ROM は読まない)
;   ・DOS モードでは F-BASIC ROM ($8000-$FBFF) が現れない (64K RAM の構成)
; となる。本 ROM は BASIC を起動させない。IPL が読めなければ参考書籍のとおり
; ブザーを鳴らして止まる。
;
; 動作:
...
;   F-BASIC ROM ($8000-$FBFF) の ROM/RAM 切替 ($FD0F バンクレジスタ。
;   参考書籍) は本 ROM では触らない。
;
; ディスク BIOS:
