...
;
; 末尾には公開 JMP テーブルと、BIOS 間接ベクタ語 $FBFA (参考書籍
; に載る) を置く。参考書籍の固定入口は各モジュールに置く。
; このベクタ語の手前には
; 当方の起動 ROM と本 ROM の内部の取り決めである目印語と起動ベクタ 2 語を置く
; (機種付属の BASIC が使うアドレスとは別に、当方が選んだアドレス)。
;==============================================================================
                ORG     $8000
...
;==============================================================================
                ...
                ; 区画の区切りは外した。互換の要件ではない内部の区切り
                ; なので、連携サービス入口の手前へ空きをまとめて 1 か所に集める。
                ...
                include "7t_strown.inc"         ; 文字列の所有 (空きプールから移設)
...
eval_to_int_round:
                ...
                include "7t_strreloc.inc"       ; スタック移設 (区画の都合でここへ)
...
;==============================================================================
; (brk_gate / brk_stop の本体は容量の都合で 7t_cmt_io.inc の空きプールへ移設)
...
;     ・新行番号は新しく付ける行番号の最初の番号。省略値は 10。
;     ・旧行番号は新しく付け始める現在のプログラムの中の行番号。省略値は
;       プログラムの最初の行番号。
;     ・増分値は新しく付ける行番号の増分値。省略値は 10。
;     ・GOTO / GOSUB / THEN / ON〜GOTO / ON〜GOSUB / ERL とともに参照して
;       いる行番号も付け直す。参照しているのにプログラム中に存在しなかった
;       行番号は `Undefined Line Number XXXXX in YYYYY` を表示し
;       (XXXXX = 存在しなかった行番号, YYYYY = その文の新しい行番号)、
;       誤っていた行番号はそのまま残す。
;     ・プログラムの中で実行したときは、その後コマンドレベルへ戻る。
;     ・63999 を越える行番号を発生することはできず、プログラムの順序が
;       変わる指定もできない。いずれも Illegal Function Call。
;
...
; 機能キー割込 (参考書籍)
;
;   参考書籍が定める書式は次の 2 つである。
;     KEY(ファンクションキー番号) { ON | OFF | STOP }
;     ON KEY(ファンクションキー番号) GOSUB 行番号
;   ファンクションキー番号は 1〜10。意味論も参考書籍の記述どおりに実装する:
;     - ON   … 割込み動作を可能にする。以後そのキーを押すと ON KEY(n) GOSUB で
;              定義した割込み処理ルーチンが呼出される
;     - OFF  … 割込みを禁止する
;     - STOP … 割込み動作を一時停止する。押しても呼出しは行わず、次に
;              KEY(n) ON を実行したときに割込みルーチンが呼出される
;              (= 発生を 1 件保留したまま持ち越す)
;     - RUN コマンドですべてのファンクションキーが OFF 状態になる
;     - 割込みルーチンからの復帰は RETURN 文。行番号があればその行から、
;       無ければ中断した個所から実行が再開される (= 通常の GOSUB 枠で入る)
;
;   ハードウェア側の取り決めは参考書籍による:
;     - PF キーを押すとメイン CPU に PF 割込みを起こせる。このとき共有 RAM
;              中のステータス (STATUS) バイトのビット 0〜3 に PF 番号がセットされる
;     - Interrupt Control (コマンドコード $2C)。パラメータ 相対 3,4 が
;              16 ビットの割込制御フラグで、下位 10 ビットが PF1〜PF10 に対応し、
;              1 のビットのキーからの割込みを受け付ける。RESET 時は $0000
;     - STATUS のビット 0〜3 が PF 割込コード。サブ側はビットを 1 に
;              するだけなので、メイン CPU は受け付けた時に割込原因のビットを 0 に
;              戻す (継続フラグビット (bit7) の内容は変更しない)
;     - ATTENTION は FIRQ 要因で、$FD04 のビット 0 に現れる (0=あり)
;
...
;   (参考書籍「Display Sub System は、STATUS バイトの各ビットを 1 に
;   セットするだけですので、メイン CPU は割込みを受付けた時、または必要でなく
;   なった時に割込原因の各ビットを 0 にする必要があります。（ただし、その時、
;   継続フラグビットの内容を変更してはいけません。）」)。破壊: A, B。
key_attn_poll:
...
;   (参考書籍「デバイス登録テーブル・ルーチンベクトル関係」)
;
;   $06EC-$078C をまず 0 に均す。0 は「その物理機番は使用しない」
;   「開いている論理機番は無い」「割込の登録は無い」を表す。
...
;   本 ROM はどちらの機能も持たないので、参考書籍の
;   「MON コマンドフック」($0269) と「TERM コマンドフック」($0284) を呼んでから
...
;   MOTOR OFF も同じ入口を使う。
;   cmt_motor_on は A = $02 を残す (元の LDA #$02 / STA と同じ)。
...
;   そのファイルの次に進んでいるので、SKIPF の「指定したファイルの次まで
;   進める」も同じ処理で満たされる。
...
;==============================================================================
; files_cas — FILES "CAS0:" (参考書籍 FILES の例)
;   テープを送りながらヘッダブロックを拾い、`ファイル名 種類 形式` を 1 行
;   ずつ並べる。終了は BREAK キー (同「カセットテープに対する FILES 文の
;   終了は、BREAK キーを使います」) か、テープの終わり。
...
ifc_have:
                ...
                ; ヘッダの種類 (+8) と形式 (+9) を、ディスクの一覧と同じ
                ;   並び (種類 = +11 / 形式 = +12) へ移してから共通の
                ;   1 行出力 (if_pe_body) に渡す。ヘッダ緩衝は 20 バイトあり、
                ;   +10 以降は照合に使わないので上書きしてよい。
                LDD     cmt_hdr_buf+8   ; +8 / +9 を 1 度に (ワード (2 バイト))
                STD     cmt_hdr_buf+11
                LDY     #cmt_hdr_buf
                JSR     if_pe_body      ; ファイル名 + 種類 + 形式
                JSR     disk_crlf
                BRA     ifc_lp
ifc_end:
...
;==============================================================================
; fileinfo_set — 解析した cmt_filename / dl_device から、参考書籍
;   ファイル名長 ($02DD) / ファイル名 ($02DE-$02E5。8 文字に満たない
;   分は空白) / 入出力デバイス物理機番 ($02E6) を埋める。物理機番は参考書籍
;   「物理機番対応表」の CAS0: = $02、フロッピーディスク 0〜3 = $80〜$83。
;   X は保存する。破壊: A, B, U
;==============================================================================
fileinfo_set:
                PSHS    X
...
;
;   参考書籍「DELETE」
;     ・行番号 1 と行番号 2 を指定したときは、その間に含まれる全ての行を削除する。
;     ・行番号 1 のみを指定したときは、その行だけを削除する。
;     ・コンマまたはハイフォンと行番号 2 のみを指定したときは、行番号 2 までの
;       全ての行を削除する。
;     ・行番号 1 とコンマまたはハイフォンを指定したときは、行番号 1 以上の
;       行番号を持つ全ての行を削除する。
;     ・コンマまたはハイフォンのみを指定したときは、全ての行を削除する。
;     ・プログラムの中で DELETE 文が現われたときは、実行後コマンドレベルへ戻る。
;   範囲の書き方は LIST と同じなので、解析は il_parse_range を共用する
...
;
;   参考書籍「UNLIST」
;     ・このコマンドの実行後は、指定された行番号を持つ行から先の表示を行わない
;       (同項の例: 10 行の UNLIST 20 を実行すると 20 行以下のリストが見えなくなる)。
;     ・アスキー形式で SAVE すると、UNLIST の実行されている行は保存できない。
;   保持先は参考書籍「UNLIST 行番号」の位置 (unlist_ln)。
;   同部のとおり LIST はこの値以上の行番号を持つ行を表示せず、解除はこの位置へ
...
;
;   参考書籍「AUTO」
;     ・行番号を最初の番号とし、増分値で増加した行番号を付ける。
;     ・行番号・増分値は 1〜5 桁の整数。行番号の指定が無いときは 10、増分値の
;       指定が無いときも 10 と見なす。
;     ・`AUTO 300,` のようにコンマだけを書いたときは、直前に実行した AUTO の
;       増分値を引き継ぐ。
;     ・63999 を越える行番号、あるいは既にある行番号のときは、CTRL+C・CTRL+X・
;       BREAK キー、または行番号の直後にリターンキーを入力することで AUTO の
;       機能は終了し、BASIC のコマンドレベルへ戻る。
;   保持先は参考書籍「AUTO フラグ / AUTO 次行番号 / AUTO 増分」
;   の位置 (auto_flag / auto_next_ln / auto_inc)。
;
...
;
;   参考書籍「EDIT」
;     ・画面がクリアされた後、指定された行が表示され、カーソルが行番号の直後で
;       点滅する。
;     ・カーソル移動キー (→ ← ↑ ↓)、INS、DEL、EL、BS で 1 行の編集ができる。
;     ・リターンキーの押下により編集動作を終了し、コマンドレベルへ戻る。
;   編集そのものはサブシステムの $04 GET (参考書籍) が
...
;   (参考書籍 FILES の出力例)。nnn は割当表で未使用 ($FF) の
;   クラスタの数 (参考書籍)。破壊: A, B, X
;------------------------------------------------------------------------------
if_free_out:
...
;   参考書籍「CLOSE」/「END」: 番号を省略した CLOSE と
...
play_tp_tbl:
                ...
                ; --- 目印語 + 起動ベクタ語 (当方の起動 ROM との内部の取り決め) ---
                ...
                ; --- BIOS の間接ベクタ語 ($FBFA) ---
                ; 参考書籍: BIOS は RCB 先頭を X に置いて
                ; JSR [$FBFA] で呼ぶ。入口は 1 ヵ所だけ (参考書籍)。
                FDB     ml_io_dispatch  ; $FBFA = BIOS 入口 (RCB を X で渡す)
                ; --- 起動のアドレス ($FBFC / $FBFE。動作観察) ---
...
;     (起動バナー後の自発再起動・ガベージ表示として動作観察で突き止めた真因)。
;     $FFFE (RESET) はブート側管理のため実行時設置でも触らない。
; かつてここに置いた FDB のベクタ表は ROM 封入範囲外に落ちる死に表であり、
; 誤解防止のため削除した (実行時設置 time_init が正)。
;==============================================================================
                ZMB     $FFF0-*
