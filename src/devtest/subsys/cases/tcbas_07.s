...
;
;   参考書籍は、機械語 I/O
;   サービスについて「入口は 1 ヵ所だけ」「RCB の先頭アドレスをインデックス
;   レジスタ X にセットし、$FBFA アドレスの間接アドレッシングを用いる
;   (`JSR [$FBFA]`)」と定める。よって呼出しは間接ベクタ語で行う
;   (実体のアドレスは参考書籍に記載が無く、呼出しの取り決めではない)。
;   RCB の割付は参考書籍 OUTPUT のとおり:
