...
;==============================================================================
; AltROMs boot_init.rom — FM77AV のディスク起動用 resident boot イメージ
;                         ($FE00-$FFFF、512 バイト)
;
; initiate ROM に埋め込まれ、ディスク起動時に $FE00-$FFFF (RAM) へ転送されて実行される。
; ディスク BIOS ($FE02 / $FE05 / $FE08) と失敗時の扱いは FM-7 の DOS モード起動 ROM
; (boot_dos) と同じで、拡張の検出 ($FD05 bit0 EXTDET) は持たない。
;
; 出典: 参考書籍 (docs/LEGAL.md §6.1 の 4 点) と動作観察 (同 §6.2)。
;
; 参考書籍の「(2) DOS (ミニフロッピィディスク)」が DOS モードのブート ROM
; の存在を定める。本イメージは BREAK キー・ディスク有無の判定を持たず、無条件に
; ディスク起動を行う。
;
; ★ IPL の置き場 (動作観察。変えてはならない): FM77AV では、起動モードに依らず
;   トラック 0 セクタ 1 を $0100 へ読み、$0100 から始める。IPL は $0100〜$02FF の
;   絶対アドレスを読み書きしてよく、$0100 以外へ置くとその参照先が食い違う。
;   $0300 を使うのは FM-7 の DOS モードの起動 ROM (boot_dos) だけである。
;
; 本イメージは DOS モードでのみ $FE00 へ転送される (BASIC モードで転送されるのは
; boot_bas)。F-BASIC ROM ($8000-$FBFF) を現す ROM/RAM 切換は $FD0F の読出であり
; (参考書籍 裏 RAM 機能)、DOS モードの経路ではこの読出が行われないため、
; 本イメージが走る時点で F-BASIC ROM は現れていない。加えて参考書籍は、この切換を
; $0000〜$7FFF のプログラムから行うことを求めており、$FE00 に置く本イメージ自身は
; 切換を行えない。よって本イメージは BASIC を起動させない。IPL が読めなければ
; ブザーを鳴らして止まる (参考書籍の DOS モードの扱いと同じ)。
;
; 動作:
;   $FE00 → 割込マスク、S・DP 設定 → ドライブ 0 のモータ ON → READY になるまで RESTOR を
;   出し直す (上限約 2 秒) → RESTOR → トラック 0 セクタ 1 を $0100 へ
;   DREAD → 成功なら Y = $0500・DP = 0 で $0100 へ。読めなければ RESTOR から
;   READ_TRIES 回までやり直し (参考書籍: BIOS は再試行しないので呼出側が再試行
;   する。再度 RESTOR を呼んでからやり直すとよい)、それでも読めなければ $FD03 に連続
;   ブザー (bit7 = 連続ブザー、bit0 = スピーカ) を書いて止まる。
;
; ディスク BIOS:
;   $FE02 RESTOR / $FE05 DWRITE / $FE08 DREAD (参考書籍)。X = RCB で
;   呼ぶ。本体 fdc_rw は RCB+0 のリクエスト番号で分岐する。AV 起動用イメージ (avboot)
;   もこの入口を呼ぶ。
;
...
IPL_ADDR        EQU     $0100           ; IPL の置き場 = 実行アドレス (トラック 0 のセクタ 1。
                                        ;   FM77AV は起動モードに依らず $0100。動作観察)
IPL_Y           EQU     $0500           ; IPL へ渡す Y (動作観察。IPL が Y を置かずに
                                        ;   使ってもよいよう、BASIC モードの
                                        ;   起動 ROM と同じ値を渡す)
                ENDC
...
rb_ready:       LBSR    spin_wait       ; 少し待つ
                LDX     #restor_rcb
                JSR     fdc_rw          ; RESTOR (A = エラー番号、Z は A に従う)
                BEQ     rb_load         ; 正常 → READY
                DEC     ,S              ; 残り回数
                BNE     rb_ready
                ; --- RESTOR → トラック 0 のセクタ 1 を $0100 へ DREAD。
...
rb_retry:       LDX     #restor_rcb
                JSR     fdc_rw          ; RESTOR
                LDX     #dread_rcb
                JSR     fdc_rw          ; DREAD セクタ 1 → $0100 (A = エラー番号、Z は A に従う)
                BEQ     rb_ok
                DEC     ,S              ; 残り回数
                BNE     rb_retry
...
rb_stop:        BRA     rb_stop
                ; --- 成功: DP = 0 で $0100 の IPL へ ---
rb_ok:          LEAS    1,S             ; 残り回数の枠を捨てる (S を初期値に戻す)
                IFNDEF  STAGE_ALT_RAM
                LDY     #IPL_Y          ; IPL へ渡す Y = $0500 (動作観察)
                ENDC
                CLRA                    ; A = 0
                TFR     A,DP            ; DP = 0
                JMP     IPL_ADDR
...
fr_type1:       LDB     4,X
                ...
                STA     <FDC_CMD
                LBSR    fdc_wait        ; BUSY 解除待ち (C=1 なら待ち切れ。$FF5E の後ろに置く)
                LDA     #ERR_TIMEOVER
                BCS     ft_ret          ; 待ち切れ → 15
...
;------------------------------------------------------------------------------
; $FF5E: ディスク BIOS の入口 (X = RCB、種別に従って処理する。動作観察)。
;   fdc_wait・spin_wait はこの後ろに置き、本体 fdc_rw の並び (+$21 / +$3D) を変えない。
;------------------------------------------------------------------------------
                ZMB     $FF5E-*
