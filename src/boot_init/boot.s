...
;
; 本イメージは BREAK キー・ディスク有無の判定を持たず、無条件に
; ディスク起動を行う。
;
...
; DOS モードの経路ではこの読出が行われないため、本イメージが走る時点で F-BASIC ROM
; は現れていない。切換は $0000〜$7FFF のコードから行うので本イメージは行わない。
; よって本イメージは BASIC を起動させない。IPL が読めなければブザーを鳴らして止まる。
;
; 動作:
...
;------------------------------------------------------------------------------
; $FF5E: ディスク BIOS の入口 (X = RCB、種別に従って処理する)。
;   fdc_wait・spin_wait はこの後ろに置く。
;------------------------------------------------------------------------------
                ZMB     $FF5E-*
